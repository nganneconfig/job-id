...
-- Enhanced: Check for new Job ID before activating auto join
...
_G.Job = ""
_G.SpamJoin = false
_G.LastJobId = ""  -- Biến mới để lưu Job ID cuối cùng đã kích hoạt
...
-- Hàm kiểm tra Job ID mới
local function isNewJobId(jobId)
    -- So sánh với Job ID cuối cùng đã kích hoạt
    return jobId and jobId ~= "" and jobId ~= _G.LastJobId
end
...
local function toggleSpamJoin()
    ...
    if not _G.SpamJoin then
        ...
        spawn(function()
            ...
            -- Chỉ bật spam join nếu có Job ID mới
            if (loadedFromClipboard or (JobIdInput.Text and JobIdInput.Text ~= "")) then
                ...
                -- Kiểm tra xem Job ID có mới không
                if isNewJobId(_G.Job) then
                    _G.SpamJoin = true
                    _G.LastJobId = _G.Job  -- Lưu Job ID mới này
                    ...
                    -- Job ID không mới, hiển thị thông báo
                    SpamJoinButton.Text = "Same Job ID"
                    SpamJoinButton.BackgroundColor3 = Color3.fromRGB(255, 165, 0)
                    SpamJoinStroke.Color = Color3.fromRGB(255, 165, 0)
                    RedIndicator.BackgroundColor3 = Color3.fromRGB(255, 165, 0)
                    
                    -- Reset về trạng thái bình thường sau 1 giây
                    spawn(function()
                        wait(1.5)
                        if not _G.SpamJoin then
                            SpamJoinButton.Text = "Spam Join"
